    ...
    )
{
  ...
  // predictor-corrector
  {
    ...
    const auto rz = cuCmul(com_z_table[dpos[2]], cp);
    atomicAdd(&com_x[n].x, rx.x); 
    atomicAdd(&com_x[n].y, rx.y); 
    atomicAdd(&com_x[n].z, rx.z); 
    atomicAdd(&com_y[n].x, ry.x); 
    atomicAdd(&com_y[n].y, ry.y); 
    atomicAdd(&com_z[n].y, ry.z);     
    atomicAdd(&com_z[n].x, rz.x); 
    atomicAdd(&com_z[n].y, rz.y); 
    atomicAdd(&com_z[n].z, rz.z); 
  }
    ...
    )
{
  ...
  if(q==0)
  {
    ...
    if(store)
    {
      double v[3] = { vel[n][0], vel[n][1], vel[n][2] };
